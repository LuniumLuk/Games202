...
let precomputeLG = [];
let precomputeLB = [];
let rotatedPrecomputeLR = [];
let rotatedPrecomputeLG = [];
let rotatedPrecomputeLB = [];
...
async function GAMES202Main() {
	...
	let maryTransform = setTransform(0, -25, 0, 25, 25, 25);
	...
	for (let i = 0; i < envmap.length; i++) {
		...
		for (let j = 0; j < 9; j++) {
			...
			precomputeLR[i].push(precomputeL[i][j][0]);
			precomputeLG[i].push(precomputeL[i][j][1]);
			precomputeLB[i].push(precomputeL[i][j][2]);
		}
	}
	rotatedPrecomputeLR = JSON.parse(JSON.stringify(precomputeLR));
	rotatedPrecomputeLG = JSON.parse(JSON.stringify(precomputeLG));
	rotatedPrecomputeLB = JSON.parse(JSON.stringify(precomputeLB));
	...
	function createGUI() {
		...
		panelModel.add(guiParams, 'envmapId', { 'GraceGathedral': 0, 'CornellBox': 1, 'Indoor': 2, 'Skybox': 3 }).name('Envmap Name');
		panelModel.open();
	}
