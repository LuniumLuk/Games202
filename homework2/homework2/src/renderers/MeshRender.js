...
class MeshRender {
	...
	bindGeometryInfo() {
		...
		if (this.mesh.hasNormals) {
			...
			const offset = 0;
			gl.bindBuffer(gl.ARRAY_BUFFER, this.#normalBuffer);
			if (this.shader.program.attribs[this.mesh.normalsName] > -1) {
				gl.vertexAttribPointer(
	...
	bindMaterialParameters() {
		...
		for (let k in this.material.uniforms) {
			...
			if (this.material.uniforms[k].type == 'matrix4fv') {
				...
				if (this.material.uniforms[k].value == 'R') {
					gl.uniformMatrix3fv(
						this.shader.program.uniforms[k],
						false,
						// precomputeLR[guiParams.envmapId]);
						rotatedPrecomputeLR[guiParams.envmapId]);
				} else if (this.material.uniforms[k].value == 'G') {
					gl.uniformMatrix3fv(
						this.shader.program.uniforms[k],
						false,
						// precomputeLG[guiParams.envmapId]);
						rotatedPrecomputeLG[guiParams.envmapId]);
				} else if (this.material.uniforms[k].value == 'B') {
					gl.uniformMatrix3fv(
						this.shader.program.uniforms[k],
						false,
						// precomputeLB[guiParams.envmapId]);
						rotatedPrecomputeLB[guiParams.envmapId]);
				} else {
					gl.uniformMatrix3fv(
						this.shader.program.uniforms[k],
						false,
