...
namespace ProjEnv
{
    ...
    float EvalSH(int k, const Eigen::Vector3f& dir) {
        switch (k) {
        case 0:
            return sh::EvalSH(0, 0, dir.cast<double>());
        case 1: 
            return sh::EvalSH(1, -1, dir.cast<double>());
        case 2:
            return sh::EvalSH(1,  0, dir.cast<double>());
        case 3:
            return sh::EvalSH(1,  1, dir.cast<double>());
        case 4: 
            return sh::EvalSH(2, -2, dir.cast<double>());
        case 5: 
            return sh::EvalSH(2, -1, dir.cast<double>());
        case 6:
            return sh::EvalSH(2,  0, dir.cast<double>());
        case 7:
            return sh::EvalSH(2,  1, dir.cast<double>());
        case 8:
            return sh::EvalSH(2,  2, dir.cast<double>());
                                                    ...
                                                    const int &channel)
    {
        ...
        constexpr int SHNum = (SHOrder + 1) * (SHOrder + 1); // 9
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);
                    // Eigen::Array3f Le(1.0f, 0.8f, 0.5f); // for Validate
                    float u = (float)x / width;
                    float v = (float)y / height;
                    float area = CalcArea(u, v, width, height);
                    float theta = acos(clamp(dir.z(), -1.0f, 1.0f));
                    sumWeight = area * sin(theta);
                    for (int k = 0; k < SHNum; k++) {
                        SHCoeffiecents[k] += Le * EvalSH(k, dir) * sumWeight;
                    }
                }
            }
        }
        std::cout << "SH Coeffs:" << std::endl;
        for (int k = 0; k < SHNum; k++) {
            for (int i = 0; i < 3; i++) {
                std::cout << SHCoeffiecents[k][i] << " ";
            }
            std::cout << std::endl;
        }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            double weight = 2.0 * M_PI / (sample_side * sample_side);
            for (int i = 0; i < mesh->getVertexCount(); i++)
            {
                ...
                auto shFunc = [&](double phi, double theta) -> double {
                    ...
                    if (H > 0.0f) {
                        ...
                        const Normal3f  n0 = mesh->getVertexNormals().col(its.tri_index.x()),
                                        n1 = mesh->getVertexNormals().col(its.tri_index.y()),
                                        n2 = mesh->getVertexNormals().col(its.tri_index.z());
                        n = bary.x() * n0 + bary.y() * n1 + bary.z() * n2;
                        ...
                        auto sh = bary.x() * sh0 + bary.y() * sh1 + bary.z() * sh2;
                        float value = 0;
                        for (int k = 0; k < SHNum; k++) {
                            value += sh.coeff(k) * ProjEnv::EvalSH(k, wi);
                        }
