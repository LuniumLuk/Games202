...
float PCF(sampler2D shadowMap, vec4 coords) {
  ...
  float shadow = 1.0;
  for (int i = 0; i < NUM_SAMPLES; i++) {
    vec4 rgbaDepth = texture2D(shadowMap, coords.xy + diskRadius * poissonDisk[i]);
    float depth = unpack(rgbaDepth);
...
float PCSS(sampler2D shadowMap, vec4 coords){
  ...
  vec3 lightDir = normalize(uLightPos);
  float bias = max(0.002 * (1.0 - dot(normal, lightDir)), 0.001);
  float penumbraSize = WIDTH_LIGHT * (zReceiver - zBlocker) / zBlocker;
  penumbraSize = max(penumbraSize, 0.001);
  // penumbraSize = 0.05;
...
float useShadowMap(sampler2D shadowMap, vec4 shadowCoord){
  ...
  return fragDepth - bias > depth ? 0.0 : 1.0;
}
